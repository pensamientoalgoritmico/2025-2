...
//Es un programa donde puedes recorrer las diversas localidades de bogotá, conocer sobre ellas y en el momento que quieras terminar el recorrido utilizas el boton de la esquina inferior derecha; alli te lleva a una selección de localidades y te genera los posters respectivos de las mismas acompañados de las frases finales, este es el frame que se descarga automaticamente como imagen.
// La Ia me ayudo con la creación de textos para los parrafos de las localidades yo una por una le proporcione las caracteristicas y contrastes que queria se hicieran visibles y le pedi que organizara unt texto, creandolos todos con la misma estructura, muy fiel a mi manera de redactar.
//Para el tema de las ilustraciones hice una preseleccioón de 18 imagenes que npo comunicaban el mismo estilo visual, por esta razón despues con ayuda de IA y un prompt muy especifico de como lo quedia una por una las fue iternando hasta lograr que se ilustraran todas con un mismo estilo
...
let estado = "bienvenida"; //Variable para definir el primer estado o momento del programa
let inputBox, botonEnviar; //Variable para crear la caja de texto y que aparezca el boton de enviar
let opinionUsuario = ""; //Variable para guardar la opinion del usuario
let mostrarOpinion = false; //Variable para iniciar el programa con esta condicion
let desplazamientoTexto = 0; //Variable para definir el desplazamiento de texto
let mostrarTexto = true; //Variable para iniciar el programa con esta condicion
let localidadActiva = ""; //Variable para que el programa reconozca que localidad esta activa
let textoLocalidad = ""; //Variable para que con ayuda de la anterior sepa que texto debe mostrar
let colorLocalidad; //Variable para definir de manera aleatoria un color para la caja de texto de cada localidad
let mostrarTerminar = true; //Variable para inciar el programa con esta condicion
let mostrarConfirmacion = false; //Variable para iniciar el programa con esta condicion
let btn = { x: 0, y: 0, s: 60 }; //Variable para definir los valores del boton (btn)
let estadoEncuesta = false; //Variable para iniciar el programa con esta condicion
let localidadesElegidas = []; //Arreglo para guardar las localidades (3) que han sido elegidas
let estadoFinal = false; //Variable para iniciar el programa con esta condicion
let imagenesLocalidades = {};
let frasesFinales = {
  //Variable que almacena todas las posibles frases finales que se pueden mostrar
  usaquen: "Donde el pasado se reinventa para\nseguir habitando el presente",
  suba: "Donde la ciudad se abre como un mosaico de contrastes",
  engativa: "Donde lo cotidiano se convierte en tejido compartido",
  fontibon: "Donde la industria respira junto a la tradición",
  barriosunidos:
    "Donde el trabajo se transforma en comunidad\ny en celebración",
  santafe:
    "Donde lo solemne se mezcla con lo\níntimo, y la ciudad se vuelve cercana",
  martires: "Donde la memoria se convierte en movimiento",
  puentearanda: "Donde la máquina y la vida se entrelazan sin pausa",
  kennedy:
    "Donde la multitud se transforma en fuerza\ncolectiva que nunca se detiene",
  ciudadbolivar:
    "Donde la esperanza se pinta en los muros y\nse mira desde lo alto",
  bosa: "Donde la raíz se mezcla con la urgencia del presente",
  tunjuelito: "Donde el esfuerzo cotidiano se convierte\nen alegría compartida",
  rafaeluribeuribe:
    "Donde la cotidianidad se convierte en resistencia creativa",
  antonionarino: "Donde lo sencillo se convierte en memoria viva",
  sancristobal:
    "Donde la montaña se convierte en barrio y\nel barrio en paisaje",
  usme: "Donde el campo se convierte en ciudad sin dejar de ser raíz",
  teusaquillo: "Donde la elegancia y la cultura se entrelazan",
};
//A continuación se muestran todas las Variables que contienen el respectivo texto que se mostrara al dar clic en la localidas (Esta fue la solución que pense cuando no funcionaba el "load strings")
let textoAntonioNarino = [
  ...
  "Para algunos, antonionarino es rutina, un lugar donde la vida cotidiana se sostiene en el comercio local,",
...
let textoBarriosUnidos = [
  ...
  "Para algunos, barriosunidos es tradición, un lugar que conserva la fuerza de la clase trabajadora y sus formas de resistencia;",
...
let textoCiudadBolivar = [
  "Ciudad Bolivar se extiende como un conjunto de ladrillos que resaltan entre la montaña,",
  ...
  "ciudadbolivar es fragilidad y orgullo,",
...
let textoEngativa = [
  "Engativa es una localidad que se despliega entre lo residencial y lo comunitario,",
  ...
  "Para algunos, engativa es rutina, un lugar de tránsito y descanso,",
...
let textoFontibon = [
  "Fontibon es un territorio que respira entre la fuerza industrial y la memoria comunitaria.",
  ...
  "Para algunos, fontibon es el rostro de la productividad,",
...
let textoMartires = [
  "Los Martires es una localidad atravesada por la memoria de la lucha y el comercio popular.",
  ...
  "Para algunos, Los martires es un lugar de memoria,",
...
let textoPuenteAranda = [
  ...
  "Para algunos, puentearanda es el engranaje de la ciudad,",
...
let textoRafaelUribeUribe = [
  ...
  "rafaeluribeuribe es barrio y movimiento, esfuerzo y transformación,",
...
let textoSanCristobal = [
  "San Cristobal es una localidad que vibra en la tensión entre la montaña y el barrio,",
  ...
  "Para algunos, sancristobal es paisaje,",
...
let textoSantaFe = [
  ...
  "santafe es contraste y superposición,",
...
let textoUsaquen = [
  "Usaquen es un territorio que respira en dos ambientes:",
  ...
  "Para algunos, usaquen es nostalgia,",
...
let textoteusaquillo = [
  "Teusaquillo es una localidad que respira entre la elegancia y la cultura contemporánea,",
...
function preload() {
  //Nos ayudamos del preload para asegurarnos de que los archivos externos se esten cargando correctamente
  titulos = loadFont("Titulos.otf"); //Cargar la tipografia para titulos
  parrafos = loadFont("Parrafos.ttf"); //Cargar la tipografia para parrafos
  //A continuación se carga una por una cada imagen con el respectivo "nombre"
  imagenesLocalidades["usaquen"] = loadImage("usaquen.png");
  imagenesLocalidades["suba"] = loadImage("suba.png");
  imagenesLocalidades["engativa"] = loadImage("engativa.png");
  imagenesLocalidades["fontibon"] = loadImage("fontibon.png");
  imagenesLocalidades["teusaquillo"] = loadImage("teusaquillo.png");
  imagenesLocalidades["barriosunidos"] = loadImage("barriosunidos.png");
  imagenesLocalidades["chapinero"] = loadImage("chapinero.png");
  imagenesLocalidades["santafe"] = loadImage("santafe.png"); // solo una vez
  imagenesLocalidades["martires"] = loadImage("martires.png");
  imagenesLocalidades["puentearanda"] = loadImage("puentearanda.png");
  imagenesLocalidades["kennedy"] = loadImage("kennedy.png");
  imagenesLocalidades["bosa"] = loadImage("bosa.png");
  imagenesLocalidades["ciudadbolivar"] = loadImage("ciudadbolivar.png");
  imagenesLocalidades["tunjuelito"] = loadImage("tunjuelito.png");
  imagenesLocalidades["rafaeluribeuribe"] = loadImage("rafaeluribeuribe.png");
  imagenesLocalidades["sancristobal"] = loadImage("sancristobal.png");
  imagenesLocalidades["usme"] = loadImage("usme.png");
  imagenesLocalidades["antonionarino"] = loadImage("antonionarino.png");
}
...
function setup() {
  ...
  //Crea y ahce visible la caja de texto para la opinión
  inputBox = createInput();
  inputBox.position(width / 2 - 150, height / 2 + 50);
  inputBox.size(300);

  //Crea y hace visible el botón para enviar y cambiar de estado
...
function draw() {
  ...
  if (estado === "bienvenida") {
    ...
    textFont(parrafos);
    text(
      "Antes de comenzar quiero que me digas brevemente\n¿Cómo definirías Bogotá? ¿Para ti qué hace a Bogotá Bogotá?",
      width / 2,
      height / 2 - 50
    );
  } else if (estado === "mapa") {
    ...
    if (mouseTX > 20 && mouseTX < 290 && mouseTY > 30 && mouseTY < 140) {
      //Define un limite para el tamaño de la localidad que será el area donde se llevara a cabo la condición
    ...
    if (mouseTX > 20 && mouseTX < 290 && mouseTY > 30 && mouseTY < 140) pop(); //Restaura el estado original en el que venia el programa antes del cambio (Es el cierre del parentesis)
    ...
    textSize(38); // Tamaño grande para la tipografía
    textAlign(RIGHT, TOP); // Alinear a la derecha y arriba (Borde superior derecho)
    text("RITMOS DE CIUDAD", width - 250, 100); // Posición con margen derecho y superior
    if (mostrarOpinion) {
      desplazamientoTexto -= 1;
      let texto = opinionUsuario + " "; //Muestra la opinion del usuario en cintas de peligro
      ...
      noStroke();
      fill(0);
      for (let x = desplazamientoTexto; x < width + 400; x += textWidth(texto))
        text(texto, x, height / 4);
      ...
      textAlign(CENTER, CENTER);
      text(
        "¿Estás listx para cambiar esta perspectiva?",
        width / 2,
        mensajeY + 40
      );
    ...
    if (!mostrarOpinion) {
      ...
      fill(0);
      text(
        "Explora la ciudad seleccionando las localidades que desees",
        30,
        height - 30
      );
      ...
      if (mostrarTexto && localidadActiva !== "") {
        ...
        if (localidadActiva === "antonionarino") {
          textoLocalidad = textoAntonioNarino.join(" ");
        }
        if (localidadActiva === "barriosunidos") {
  ...
  if (estadoFinal) {
    ...
    textSize(42);
    fill(0);
    text("RITMOS DE CIUDAD", width / 2, 80);
    ...
    let espacioEntre = width / localidadesElegidas.length;
    let imagenY = height / 2 - 20; // posición vertical de las imágenes
    let nombreY = imagenY + 120; // nombre debajo de la imagen
    let fraseY = nombreY + 50; // frase debajo del nombre

    for (let i = 0; i < localidadesElegidas.length; i++) {
      let localidad = localidadesElegidas[i];
      let frase = frasesFinales[localidad];
      let imagen = imagenesLocalidades[localidad];

      let x = espacioEntre * i + espacioEntre / 2;

      // Imagen de la localidad
      if (imagen) {
        imageMode(CENTER);
        image(imagen, x, imagenY, 220, 200); // ← tamaño ajustado
      }
      ...
      textSize(24);
      fill(0);
      text(localidad.toUpperCase(), x, nombreY);

      // Frase poética
      textFont(parrafos);
      textSize(16);
      fill(50);
      text(frase, x + 15, fraseY);
    }
    // Guardar el frame final automáticamente
    save("frame_final.png");

    // Detener el loop para que no siga guardando infinitamente
    noLoop();
  }
}
...
function mousePressed() {
  ...
  // rafaeluribeuribe
  if (mouseTX > 510 && mouseTX < 620 && mouseTY > 240 && mouseTY < 290) {
    localidadActiva = "rafaeluribeuribe";
    colorLocalidad = color(
      random(180, 255),
      random(180, 255),
      random(180, 255)
